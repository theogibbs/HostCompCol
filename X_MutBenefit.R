...
ch <- 0.5
chm <- seq(0.5 + 1E-10, 6, length.out = 100) # needs to be bigger than or equal to ch
dh <- 1
dhp <- 0
...
cm_vec = c(12.75, 10.25, 7.75, 5.25)
cm <- cm_vec
dm <- 1
...
# Create trajectories from exploring initial condition state space
ini_resolution = 100
LHS_mat = randomLHS(ini_resolution, 3)
ini_states <- list(h = LHS_mat[,1], p = LHS_mat[,2], m = LHS_mat[,3]) %>% 
  filter(p + m < h)
...
out_data <- foreach(i = 1:nrow(iterated_params),
                    .combine = 'rbind') %:%
  foreach(j = 1:ini_resolution,
  .combine = 'rbind',
  .inorder = FALSE) %dofuture% {
    # for(ini_cond_sd in ini_cond_sds) {
      ...
      out_dyn <- out_dyn %>%
        filter(time > (end_time - 201)) %>% # Only plot the last 100 time points
        melt(id.vars = c("time"))
...
proc_data$cm_label <- factor(proc_data$cm_label,
                             levels = c("c[m] == 12.75", "c[m] == 10.25", "c[m] == 7.75", "c[m] == 5.25"))
...
plMutBenefit <- proc_data %>% 
  filter(!is.nan(value) & value < 1+1e-10 & value > -1e-10) %>%
  ggplot(aes(x = chm, y = value)) +
  facet_grid(
    rows = vars(cm_label),
    cols = vars(variable),
    labeller = label_parsed
  ) +
  geom_point(size = 2, color = "black", alpha = 0.005) +
...
ch <- c(0.5, 1.5, 2.5)
chm <- seq(0.5, 15, length.out = 200) # needs to be bigger than or equal to ch
dh <- 1
dhp <- 0
...
plHeatMap <- ggplot(out_data,
  ...
  labs(x = expression(atop("Added Host Colonization", "from the Mutalist" ~ (c[hm]))),
       ...
       fill = "") +
  facet_wrap(~ch, labeller = label_bquote(rows = c[h] == .(ch)), nrow = 3) +
  ggtitle("B") + 
...
jpeg("./figs/Alt_Fig3MutBenefit.jpeg",
     width = 3500, height = 2000, res = 300)
# KD: place heatmaps below trajectory plots, to be consistent with other graphs.
# KD: reduce number of heatmaps?
